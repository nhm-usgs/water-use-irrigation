...
for date_idx in range(0, 1): #len(dates)):
    pkwater_equiv_vals = vals[date_idx, :]
    ...
    gdf.plot(column='pkwater_equiv', cmap='coolwarm', ax=ax, edgecolor="face",
             norm=colors.LogNorm(vmin=limit, vmax=max(pkwater_equiv_vals)))

    ax.axis('off')
    # add a title
    ax.set_title('pkwater_equiv: ' + str(dates[date_idx])[:10], fontdict={'fontsize': '25', 'fontweight': '3'})
    # create an annotation for the data source
    ax.annotate('ONHM, PRMS v5.1.0, GFv1.1, Units: inches', xy=(0.1, .08), xycoords='figure fraction',
